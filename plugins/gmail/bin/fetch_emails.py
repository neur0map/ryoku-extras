...
def main():
    ...
    if label_upper == "INBOX":
        if flags_arg and "," in flags_arg:
            ...
            if len(flags) == 3:
                u, p, s = flags[0] == "1", flags[1] == "1", flags[2] == "1"
                if u and p and s:
                    # All three categories enabled: show everything in INBOX
                    query_params = f"labelIds=INBOX&maxResults={max_results}"
                else:
                    # User customized categories: always include Primary, plus any explicitly enabled category
                    cats = ["category:primary"]
                    if u: cats.append("category:updates")
                    if p: cats.append("category:promotions")
                    if s: cats.append("category:social")

                    if len(cats) == 1:
                        q_cats = "category:primary"
                    else:
                        q_cats = "{" + " ".join(cats) + "}"
