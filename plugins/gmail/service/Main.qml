...
Item {
    ...
    onUiActiveChanged: {
        if (uiActive) {
            console.log("[Gmail Service] Opened in view: activating on-demand");
            if (!root.credentialsConfigured) {
                root.checkCredentials();
            }
            if (!root.accounts || root.accounts.length === 0) {
                accountsLoader.running = false;
            ...
            console.log("[Gmail Service] Closed / not in view: suspending timers and killing fetchers");
            retryRefreshTimer.stop();
            debounceSyncTimer.stop();
            autoRefreshTimer.stop();

            // Immediately terminate any running background processes
            ...
            emailBodyFetcher.running = false;
        }
    }
    ...
    Process {
        ...
        command: ["python3", root.binDir + "/check_credentials.py"]
    ...
    function _onCategoryFilterChanged() {
        root.historyId = "";
        if (root.authenticated) {
            root.syncLabel("inbox", 0, true);
        }
    }
    onEnableUpdatesChanged: _onCategoryFilterChanged()
    onEnablePromotionsChanged: _onCategoryFilterChanged()
    onEnableSocialsChanged: _onCategoryFilterChanged()
    ...
    Timer {
        ...
        running: root.authenticated && root.uiActive && root.refreshIntervalMinutes > 0
        repeat: true
        onTriggered: {
            if (root.uiActive) {
                root.syncLabel("inbox");
            }
        }
    }
    ...
    Timer {
        ...
        repeat: false
        onTriggered: {
            if (root.uiActive && !root.authenticated && root._refreshToken !== "") {
    ...
    Connections {
        target: typeof Network !== "undefined" ? Network : null
        function onWifiStatusChanged() {
            if (typeof Network !== "undefined" && Network && Network.wifiStatus === "connected" && !root.authenticated && root._refreshToken !== "") {
                root._refreshAndFetch();
            }
        }
    }
    ...
    Process {
        id: accountsLoader
        command: ["python3", root.binDir + "/accounts_store.py", "get"]
        stdout: StdioCollector {
            onStreamFinished: {
                try {
    ...
    Process {
        ...
        stdout: StdioCollector {
            ...
            onStreamFinished: {
                ...
                try {
                    ...
                    root._accessToken = tok;
                    root._tokenExpiry = Math.floor(Date.now() / 1000) + (data.expires_in || 3600) - 60;
                    root.authenticated = (typeof tok === "string" && tok.length > 0);
                    if (root.authenticated) {
                        console.log("[Gmail Service] Authenticated successfully, fetching mailboxes...");
                        root._startFetchAll();
                    }
                } catch (e) {
                    root.authenticated = false;
        ...
        onExited: (exitCode, exitStatus) => {
            if (exitCode !== 0) {
                if (!root.authenticated && root.uiActive) {
    ...
    Process {
        id: authProcess
        command: ["python3", root.binDir + "/oauth_server.py"]
        stdout: StdioCollector {
            onStreamFinished: {
                try {
                    let data = JSON.parse(text);
                    if (data && data.success) {
                        console.log("[Gmail Service] OAuth finished successfully for:", data.email);
                        root._tryInit();
                    }
                } catch(e) {}
            }
        ...
        onExited: (exitCode, exitStatus) => {
            ...
            console.log("[Gmail Service] authProcess finished with code:", exitCode);
            root._tryInit();
        }
        onRunningChanged: {
            if (!running)
    ...
    property string _fetchScript: root.binDir + "/fetch_emails.py"
    ...
    function deleteEmail(messageId, mode) {
        if (mode === "permanent") {
            deleteMessagePermanent(messageId);
        } else if (mode === "untrash") {
