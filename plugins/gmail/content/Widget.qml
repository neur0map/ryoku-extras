...
Item {
    ...
    // Popup size configuration: reads live from Ryoku Settings -> Plugins -> Gmail
    readonly property var pSettings: (pluginApi && pluginApi.pluginSettings) ? pluginApi.pluginSettings : ({})
    property real defaultWidth: (pSettings && pSettings.width && Number(pSettings.width) > 0)
        ? Number(pSettings.width) : 680
    property real defaultHeight: (pSettings && pSettings.height && Number(pSettings.height) > 0)
        ? Number(pSettings.height) : 520

    readonly property var service: pluginApi ? pluginApi.mainInstance : null
    readonly property real contentW: widthBudget > 0 ? widthBudget : defaultWidth * s

    implicitWidth: defaultWidth * s
    implicitHeight: defaultHeight * s

    Component.onCompleted: {
        if (root.service) {
            EmailService.instance = root.service;
            root.service.uiActive = root.active;
        }
    }
    Component.onDestruction: {
        if (root.service) {
            root.service.uiActive = false;
        }
    }
    onServiceChanged: {
        if (root.service) {
    ...
    RowLayout {
        ...
        Item {
            ...
            EmailAuth {
                id: emailAuth
                anchors.fill: parent
                visible: !EmailService.authenticated || root.activeTab === "setup_guide"
            }
