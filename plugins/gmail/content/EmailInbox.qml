...
Item {
    ...
    StyledFlickable {
        ...
        ColumnLayout {
            ...
            // Empty states dressed with Ryoku Specimen Decor
            Item {
                ...
                ColumnLayout {
                    ...
                    RyokuDecor {
                        ...
                        art: (root.activeTab || "").toLowerCase() === "trash" ? "cube.gif"
                            : ((root.activeTab || "").toLowerCase() === "spam" ? "compass.gif"
                            : ((root.activeTab || "").toLowerCase() === "sent" ? "render.gif"
                            : ((root.activeTab || "").toLowerCase() === "search" ? "disc.gif" : "wave.gif")))

                        code: (root.activeTab || "").toLowerCase() === "trash" ? "TRASH-00"
                            : ((root.activeTab || "").toLowerCase() === "spam" ? "SPAM-00"
                            : ((root.activeTab || "").toLowerCase() === "sent" ? "SENT-00"
                            : ((root.activeTab || "").toLowerCase() === "search" ? "QUERY-00" : "INBOX-00")))

                        title: (root.activeTab || "").toLowerCase() === "trash" ? "破棄"
                            : ((root.activeTab || "").toLowerCase() === "spam" ? "検疫"
                            : ((root.activeTab || "").toLowerCase() === "sent" ? "送信済"
                            : ((root.activeTab || "").toLowerCase() === "search" ? "検索" : "平穏")))

                        sub: (root.activeTab || "").toLowerCase() === "trash" ? "TRASH PURGED"
                            : ((root.activeTab || "").toLowerCase() === "spam" ? "NO JUNK DETECTED"
                            : ((root.activeTab || "").toLowerCase() === "sent" ? "OUTBOX EMPTY"
                            : ((root.activeTab || "").toLowerCase() === "search" ? "NO RESULTS" : "INBOX ZERO")))

                        caption: (root.activeTab || "").toLowerCase() === "trash" ? "Trash bin is cleared. Items are permanently erased after 30 days."
                            : ((root.activeTab || "").toLowerCase() === "spam" ? "Zero junk or suspicious emails identified by filters."
                            : ((root.activeTab || "").toLowerCase() === "sent" ? "No outbound emails sent from this device yet."
                            : ((root.activeTab || "").toLowerCase() === "search" ? "No emails match your query. Try broadening keywords." : "All messages processed. No pending unread emails.")))

                        readout: (root.activeTab || "").toLowerCase() === "trash" ? ["STORAGE|CLEAN", "COUNT|0"]
                            : ((root.activeTab || "").toLowerCase() === "spam" ? ["FILTER|ACTIVE", "JUNK|0"]
                            : ((root.activeTab || "").toLowerCase() === "search" ? ["RESULTS|0", "QUERY|IDLE"]
                            : ["STATUS|SYNCED", "UNREAD|0", "QUEUE|CLEARED"]))

                        seal: (root.activeTab || "").toLowerCase() === "trash" ? "消"
                            : ((root.activeTab || "").toLowerCase() === "spam" ? "護" : "便")
                    }

                    // Contextual Action
                    Rectangle {
                        Layout.alignment: Qt.AlignHCenter
                        Layout.preferredWidth: ctaLabel.implicitWidth + 32
                        Layout.preferredHeight: 32
                        radius: Appearance.rounding.normal
                        color: ctaMouse.pressed ? Appearance.colors.colPrimaryActive
                            : (ctaMouse.containsMouse ? Appearance.colors.colPrimaryHover : Appearance.colors.colPrimary)
                        border.width: 1
                        ...
                        RowLayout {
                            ...
                            MaterialSymbol {
                                text: (root.activeTab || "").toLowerCase() === "spam" ? "sync" : "edit"
                                iconSize: 14
            ...
            Repeater {
                ...
                delegate: Item {
                    ...
                    Item {
                        ...
                        // Email Card (Ryoku Style)
                        Rectangle {
                            id: backgroundRect
                            width: parent.width
                            height: parent.height
                            antialiasing: true
                            color: visualPressed ? Appearance.colors.colLayer1Active : (mouseArea.containsMouse ? Appearance.colors.colLayer1Hover : (model.unread ? Appearance.colors.colLayer1 : Appearance.colors.colLayer0))
                            border.width: 1
                            border.color: mouseArea.containsMouse ? Appearance.colors.colOutline : (model.unread ? Appearance.colors.colPrimary : Appearance.colors.colOutlineVariant)
                            radius: Appearance.rounding.normal
                            ...
                            property bool isAbovePressed: false
                            property bool isBelowPressed: false

                            readonly property real swipeEffect: 0
                                    ...
                                    ColumnLayout {
                                        ...
                                        RowLayout {
                                            ...
                                            Item {
                                                ...
                                                Layout.preferredWidth: dateText.implicitWidth
                                                ...
                                                StyledText {
                                                    ...
                                                    font.pixelSize: 11
                                                    font.weight: model.unread ? Font.DemiBold : Font.Normal
                                                    color: model.unread ? Appearance.colors.colPrimary : Appearance.colors.colSubtext
                                                }
                                            }
                                        }
