...
Item {
    ...
    StyledFlickable {
        ...
        ColumnLayout {
            ...
            ColumnLayout {
                ...
                // OAuth Setup Guide & API Keys button
                Rectangle {
                    id: guideBtn
                    Layout.fillWidth: true
                    Layout.preferredHeight: 56
                    radius: Appearance.rounding.full
                    color: guideMouse.pressed ? Appearance.colors.colSurfaceContainerHighestActive : (guideMouse.containsMouse ? Appearance.colors.colSurfaceContainerHighestHover : Appearance.colors.colSurfaceContainerHigh)
                    border.width: 1
                    border.color: Appearance.colors.colOutlineVariant

                    Behavior on color {
                        animation: Appearance.animation.elementMoveFast.colorAnimation.createObject(guideBtn)
                    }
                    ...
                    RowLayout {
                        ...
                        StyledText {
                            text: Translation.tr("Google OAuth Setup Guide & API Keys")
                            font.pixelSize: Appearance.font.pixelSize.large
                            font.weight: Font.DemiBold
                            color: Appearance.colors.colOnSurface
                        }
                    ...
                    MouseArea {
                        ...
                        cursorShape: Qt.PointingHandCursor
                        onClicked: {
                            emailAuth.forceEditMode = true;
                            root.parent.activeTab = "setup_guide";
                        }
                    }
                }
    ...
    component EmailNavOrderEntry: Rectangle {
        id: entryWrapper
        Layout.fillWidth: true
        Layout.preferredHeight: 48
        radius: Appearance.rounding.small
        color: index % 2 == 0 ? Appearance.colors.colLayer3 : Appearance.colors.colLayer2
        border.width: 1
        border.color: Appearance.colors.colOutlineVariant
        ...
        RowLayout {
            ...
            // Up / Down arrow buttons for instant reordering
            RowLayout {
                spacing: 6
                ...
                Rectangle {
                    ...
                    height: 32
                    radius: Appearance.rounding.small
                    color: upMouse.pressed ? Appearance.colors.colLayer1Active
                        : (upMouse.containsMouse ? Appearance.colors.colLayer1Hover : Appearance.colors.colLayer1)
                    border.width: 1
                    border.color: Appearance.colors.colOutlineVariant
                    ...
                    MouseArea {
                        ...
                        onClicked: {
                            var fromIdx = entryWrapper.index;
                            if (fromIdx > 0) {
                                var list = EmailService.navOrder.slice();
                                var item = list.splice(fromIdx, 1)[0];
                                list.splice(fromIdx - 1, 0, item);
                                EmailService.setNavOrder(list);
                            }
                        }
                    }
                }
                ...
                Rectangle {
                    ...
                    height: 32
                    radius: Appearance.rounding.small
                    color: downMouse.pressed ? Appearance.colors.colLayer1Active
                        : (downMouse.containsMouse ? Appearance.colors.colLayer1Hover : Appearance.colors.colLayer1)
                    border.width: 1
                    border.color: Appearance.colors.colOutlineVariant
                    opacity: index < (EmailService.navOrder ? EmailService.navOrder.length - 1 : 0) ? 1.0 : 0.25
                    enabled: index < (EmailService.navOrder ? EmailService.navOrder.length - 1 : 0)
                    ...
                    MouseArea {
                        ...
                        onClicked: {
                            var fromIdx = entryWrapper.index;
                            if (EmailService.navOrder && fromIdx < EmailService.navOrder.length - 1) {
                                var list = EmailService.navOrder.slice();
                                var item = list.splice(fromIdx, 1)[0];
                                list.splice(fromIdx + 1, 0, item);
                                EmailService.setNavOrder(list);
